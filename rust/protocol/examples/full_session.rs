...
//!   2. Bob publishes a prekey bundle (signed X25519 ratchet key + signed ML-KEM-1024 prekey + a
//!      one-time ML-KEM-1024 prekey, all authenticated by his ML-DSA identity).
...
fn main() -> Result<(), SignalProtocolError> {
    // ML-KEM-1024 and ML-DSA-87 values are large, and driving the whole protocol as a single async
...
async fn run() -> Result<(), SignalProtocolError> {
    ...
    println!(
        "=== Signal session with a post-quantum initial handshake \
         (ML-KEM-1024 + ML-DSA-87 identity) ===\n"
    );
    ...
    println!(
        "[2] Bob published a prekey bundle (every item signed with ML-DSA-87):\n      \
         ...
         - signed ML-KEM-1024 prekey + one-time ML-KEM-1024 prekey  (the sole source of the shared secret)\n    \
         Signed ML-KEM-1024 public key: {} B.",
...
async fn create_pre_key_bundle<R: Rng + CryptoRng>(
    ...
    let signed_kem_pair = kem::KeyPair::generate(kem::KeyType::MLKEM1024, &mut *csprng);
    ...
    let one_time_kem_pair = kem::KeyPair::generate(kem::KeyType::MLKEM1024, &mut *csprng);
    let one_time_kem_signature = identity
        .private_key()
        .calculate_signature(&one_time_kem_pair.public_key.serialize(), &mut *csprng)?;

    let device_id = 1u32;
    let signed_pre_key_id = 1u32;
    let signed_kem_id = 1u32;
    let one_time_kem_id = 2u32;
    ...
    .with_one_time_kyber_pre_key(
        one_time_kem_id.into(),
        one_time_kem_pair.public_key.clone(),
        one_time_kem_signature.to_vec(),
    );
