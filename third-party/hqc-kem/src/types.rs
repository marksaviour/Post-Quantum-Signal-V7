    ...
    ($name:ident, $bytes:expr, $err:ident) => {
        impl<P: HqcParams> TryFrom<&[u8]> for $name<P> {
            ...
            fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
                // PQS-V7 fix: upstream hqc-kem 0.1.0 (@8f1fd8c) inverted this length
                // check (it used `==`), so correctly-sized keys/ciphertexts were
                // rejected on deserialization. The guard must reject a *wrong* length.
                if bytes.len() != $bytes {
